...
4. 构造 `MainWindowController(providerStore:)` 并 `show()` 显示主窗口。
5. `store.refresh()` 触发首次全量刷新（此时窗口/状态栏均已就绪，时序可控）。
...
- **NSWindow**：1120×720，`contentRect`，`.underWindowBackground` vibrancy（`NSVisualEffectView` material = `.underWindowBackground`，`blendingMode: .behindWindow`），`styleMask: [.titled, .fullSizeContentView, .closable, .miniaturizable]`，`titlebarAppearsTransparent = true`，`titleVisibility = .hidden`。
...
- **生命周期**：`show()` 调 `makeKeyAndOrderFront` + `NSApp.activate(ignoringOtherApps: true)`。窗口持有为强引用保活；`applicationShouldHandleReopen` 在窗口被关后重新 `show()`。

## `MainWindowView` — 主窗口内容

`NavigationSplitView`，sidebar + detail：

- **Sidebar**：5 项（概览 / 会话 / 供应商 / 用量 / 设置）。选中项用 `matchedGeometryEffect` 滑动 pill 指示。每项 trailing 带实时计数 badge（`contentTransition(.numericText())`）。顶部 CB logo（`Theme.accent`/`Theme.accentDim` 渐变）。
- **Detail**：按选中页切换 `DashboardView` / `SessionsView` / `ProvidersView` / `UsageView` / `SettingsView`，配 `.asymmetric` transition。背景叠 `AuroraBackground`（MeshGradient 漂移）+ `CursorSpotlight`（指针跟随光晕）。
- **CommandPalette**（⌘K）：跨页面模糊搜索导航。
