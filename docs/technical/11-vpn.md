...
| 菜单栏 popup | `PanelHeader` 的 VPN chip（`VpnNodePickerPanel`：选节点、测速）；VPN 运行时 status item 显示双行 ↓/↑ + 电池格（`VpnMenuBarRateView`，宽度由布局常量推导；电池格是 34×21 的无正极头胶囊，φ²∶1 ≈ 1.618∶1，比例见 `batteryGlyphWidth` 的注释） |
