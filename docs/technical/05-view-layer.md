...
- `OrbitGauge`（额度表盘：trim 弧 + 沿弧走的圆点）、`ConveyorBelt`（扫描中那种"正在持续做事"的走带，`DecorativeMotion.kind == .conveyor`）、`ShineSweep` + `.depthTilt()`（一次性扫光与 3D 倾斜，**只给单张 hero 卡**，不进 `.tile()`）。这里**已经没有 `LoadRing`**：它曾是唯一按读数调速的装饰（一条光的弧绕在机器图标背后，转速 ∝ 读数，<5% 完全静止，读数变化时用 `timeOffset` 就地重定时），但约 96° 的弧在 22–28pt 上读起来就是「转圈 = 等待」，而且它复述了下方三行已经印出的数字 —— 视图与 `DecorativeMotion.loadRing` 一并删除。实时读数改由右侧的大 mark 承担：**CPU 每个逻辑核心一格（空闲核保留浅色 socket，所以 12 核始终可数）、GPU 每组图形子单元一列，各自按自己的读数点亮**（见 `HardwareIllustration` 与 [DESIGN.md](../../DESIGN.md) 的 Machine marks）。
...
- `ProviderTile`：供应商瓦片（自适应网格；`dense` 切换密度）。**当前未挂载** —— 主窗口「模型」页现由 `ProviderDirectoryHost` + `ProviderConnectionEditor` 承担，见 [17](17-ui-audit-backlog.md)。瓦片头 = Provider 名 + 活跃胶囊 + chevron；激活瓦片顶部 48×3pt accent 胶囊 + 内描边。模型名匹配用 case-insensitive（settings.json 大小写可能不同）。
- `popup` 的模型切换走 `PanelHeader` 的 chip → `ModelSwitchList`，不用瓦片网格；`PopupModelTile` 已删除。
- `formatContext`：`200000 → 200K`、`1000000 → 1M`。

## `ProviderEditorView` — 编辑视图（**当前未挂载**）

编辑器视图没有独立窗口，也**没有调用点**：主窗口切到「模型」页后渲染的是 `ProviderDirectoryHost`，编辑走 `ProviderConnectionEditor` 弹窗。以下描述的是源码里这套编辑器本身（`ProviderEditorModel`，`@Observable`，含 `EditableModel` 本地副本、校验与保存 spinner）。左 220pt Provider 列表（`List(selection:)` + 增/删/复制），右侧 master-detail：
