...
- **本机负载的大图标会按核心/子单元实时点亮**（`HardwareIllustration`）：概览的资源条上，CPU 的芯片插画现在是**每个逻辑核心一格**，按该核心自己的占用率点亮（12 核就是 12 格，空闲核保留浅色 socket，所以机器闲着也能数出核数；采样器第二个 tick 之前没有逐核基线，此时退回整片发光）；GPU 换成同一套 100×76 网格上的显卡插画，**每组图形子单元一列**，各自按自己的读数从底部填充；内存 / 硬盘改成容量模组 + 页类别（活动 / 锁定 / 压缩）与已用 / 空闲的等比条。随鼠标指针的浮层用同一个 mark，文案也从「不代表单个核心的独立读数」改为如实描述。
- **删掉机器图标背后转圈的弧**（`LoadRing`）：CPU / GPU / 内存 / 硬盘四格与 popup 本机负载条的每一格（含耳机格），小图标背后那条转速 ∝ 读数的光弧全部移除，视图与 `DecorativeMotion.loadRing` 装饰 kind 一并删除。约 96° 的弧在 22–28pt 上读起来就是「转圈 = 等待」，而机器在干活时没有一个格子是在等待；它还复述了下方三行已经印出的数字，字更小、对比更低。实时读数的位置改由右侧的大 mark 承担。
...
- **本机负载的图标会转了**（`LoadRing`）：概览的资源条与 popup 的本机负载条上，CPU / GPU / 内存 / 硬盘 与耳机格都变成「机器图标 + 背后一条光的弧」，**弧的转速正比于这一格的读数**——CPU 47% 约 2.6 秒一圈、100% 约 0.7 秒、低于 5% 完全静止（弧停在原地，不是消失）。内存与硬盘按"用了多少"的比例转（不是字节数），耳机按"剩得越少转得越快"。和风扇同一条纪律：**转速是读数，不是装饰**——颜色仍然只由 `StatusPill` / 温度 / 压力决定，转得快不会变红。整条负载条现在只有一种动效语言（风扇仍用自己的转子，因为它本来就说同一件事）。
- **额度表盘改为沿弧走的圆点**（`OrbitGauge`）：`CodexQuotaGauges` 由「圆环 + trim」换成 300° 弧轨 + 一个站在当前读数上的圆点。
...
- 新增 `Sources/ClaudeBar/Views/Shared/UiverseSurfaces.swift`；`Tests/rendering-regressions.py` 的动效矩阵由 5 种增到 7 种（补 `conveyor` 与 `loadRing`，按条带 / 弧取景），并为 `loadRing` 补了四条判据：弧必须是弧（不是闭合环）· 空闲必须**恰好为 0**（不是"慢"）· 转速随读数上升且两端比值 > 1.5（单一定速会被抓到）· 改速率必须就地重定时而不重建图层。这四条是必需的：动效矩阵本身只传 `active`、从不传 `level`，所以一个**完全忽略读数**的实现能通过它之外的每一条断言。
