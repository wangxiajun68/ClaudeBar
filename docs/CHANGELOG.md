...
- **本机负载的 mark 换成 Lucide 官方图标 + 独立读数条**：CPU / GPU / 内存 / 硬盘现在分两条 lane —— 上层是 Lucide 自己的 `cpu` / `gpu` / `memory-stick` / `hard-drive` 图标（由 `Tools/gen-lucide-hardware.py` 从上游 SVG 生成到 `Views/Shared/LucideHardwareGeometry.swift`，`Sources/Licenses/Lucide.txt` 随包分发），下层是读数条：**CPU 每个逻辑核心一条、GPU 每组图形子单元一条、内存按页类别、硬盘按已用/空闲，条的高度就是它自己的读数**。CPU 有几核就有几条，所以 12 核可以数出来、6 核忙就是 6 条满格。图标负责「这是哪个部件」，读数条负责「它有多忙」；两条 lane 分开是因为把读数塞进图形里会互相打架（条会穿过 GPU 的接口圆和内存的芯片窗口）。另有一条按读数调速的光扫过读数条：满载约 0.6 秒一次、轻载约 2.9 秒，**低于 4% 完全静止**（和风扇 `rpm >= 80` 同一条纪律），开启减弱动效或窗口不可见时同样停止。
- **删掉机器图标背后转圈的弧**（`LoadRing`）：CPU / GPU / 内存 / 硬盘四格与 popup 本机负载条的每一格（含耳机格），小图标背后那条转速 ∝ 读数的光弧全部移除，视图与 `DecorativeMotion.loadRing` 装饰 kind 一并删除。约 96° 的弧在 22–28pt 上读起来就是「转圈 = 等待」，而机器在干活时没有一个格子是在等待；它还复述了下方三行已经印出的数字，字更小、对比更低。实时读数的位置改由右侧的 mark 承担。
