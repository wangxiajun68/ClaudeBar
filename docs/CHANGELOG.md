...
- **连接卡片的图标收到小组尺寸**：概览资源条上「连接」卡里那四颗 mark（Wi-Fi 环、隔空投送、以太网、耳机环）此前是 48pt 的 ring——整条资源条上最大的单个物件，比它自己那格 26pt 的标题徽章还大，比一行连接状态需要说的话都大。现在 mark 组收到 **38pt**（列宽 80 → 72，`labelBand` 不动，所以标题基线、两行副文、上方「已连接」胶囊和卡片高度全都不变），菜单栏 popup 的 24 → 24 不变（本来就是 popup 尺寸）。**顺带修掉一处不一致**：这张卡的标题徽章没写尺寸，于是它吃了 `InstrumentBadge` 的 24pt 默认值，而全条另外五个 meter 都显式写 26——同一条条上两种徽章尺寸。现在六格一致。耳机部件从 `dial` 推导（0.42×），所以耳机环、耳机、充电盒一起缩小，左右两只仍然对称。
- **「连接」详情重做成仪表盘，不再是五块灰字**：
  - 旧版是「路径图 + 信号条 + 代理卡 + 设备网格 + 底部说明」五块叠着，本质是一张**表单**——MAC、IP、两个 DNS 排成 mono 一列，对不 debug 的人什么都没说，而且读了四遍。现在整块是**一台机器加四只环**，打开先给**一个答案**：标题从「连接」换成链路本身的质量（Wi-Fi 上是「很强」，下面写「−46 dBm」并标出 −100 弱 / −40 强的刻度；以太网上是「已接入」）。
  - 四只环各自回答一件事，并且各自说**一个带颜色的词**（蓝＝这一跳走的路，紫＝本机自己的进程 / 挂在上面的设备）：**出口**（Wi-Fi 或以太网，弧长就是信号位置）、**本机代理**（唯一的活物：监听时那 8pt 点在呼吸，点一下即检测）、**隔空投送**、**蓝牙**（副文写设备数；没有设备时**写明原因**，空格子会被读成加载中）。三只环三种不同的画法——同心弧换四种颜色只会读成「同一只表画了四遍」，而这里四者最不同的地方恰好是「哪一种门」。
  - 那四个查不出所以然的数字没有删掉，是**归档**了：底部「复制诊断」把它们整理成一段可以直接贴进 issue 或聊天窗口的文本。底部去掉那句「更详细的路由与吞吐在 流量 页；系统级的接口列表在 macOS 的网络设置里。」——面板自己就是答案，解释自己在哪里没有意义。
  - 动效只留一个**连续的轻微扭转**：四只环按 +4°/+2°/−2°/−4° 静置，指针进入整组时一起转到 −3°，像一块铰接面板被转向你。它是 hover 状态不是循环（不会一直动），没有逐环延迟所以不需要 stagger，而且光标停在组里任何位置都不会抖。开启减弱动效则固定在静置角度。
  - 结构上加了 `Tests/connection-panel-regressions.py`：钉住「重复诊断行不折叠成一行」「四只环都在」「没有旧版的 mono 地址行」，见测试本身。
- **流量日志的 token 列对齐**：每条记录的合计、入、出、缓存、写入各占一列固定宽度，缺的显示「—」。不再写成 `Σ 25.4万 (in … / out … / hit …)` 这种长短不一的一行。
- **设置按用途重新分组，短卡片变矮**：界面（主题、单位、开机自启、继续会话）、灵动岛、花费、本机（电池、SQLite、日志）各自一行。配置文件从五张大卡收成一张列表。关于和退出并成一条。内容少的卡片不再预留三行说明的高度。
- **用量图表按范围重画，并加上「全部」**：周期条在日 / 月 / 年之外有「全部」（查询从 2020 到明天，热力图只画真正有记录的日子）。来源是一根占满 100% 的分段条，宽度就是份额，下面一行写绝对数量。节奏图跟着范围换刻度：日是这一周、月是每一天、年是十二个月、全部超过两年则按年；峰值标在最高的一柱上，虚线是均值。构成卡把缓存命中率放到大数字上。三张图只在换时间范围时做一次弹簧，用量数字自己跳动时柱子不动。全部记录没有上一周期 / 下一周期。
- **菜单栏电池去掉正极头，胶囊收回黄金比例**：那截 2.4pt 正极头不承载任何读数（电量是液面、状态是图标、数字在右边），却把胶囊的视觉重心顶偏，看着像被右边裁掉一块。删掉之后胶囊坐到整格中央，比例定成 **34×21 = φ²∶1 ≈ 1.618∶1**——胶囊本身就是黄金矩形，宽度由高度推导。**比例是这里的读数**：1∶1 是圆点，超过 2.2∶1 就是一根条。中间那版 45×18（2.5∶1）正落在「条」这一侧，液面两边各空出 3.5mm 椭圆，读起来是「一个装不满的容器」，所以又收窄了 11pt。旧尺寸 23.4×13.4 是 1.75∶1（名片比例），在 11pt 圆体数字旁边像一枚凑上去的色块。整格宽度仍从图形常量推导，`Tests/menubar-strip-regressions.py` 现在锁住高度、1.618 比例、比例上限与液面跑道。
- **动作按钮不再是系统玻璃按钮**：设置、VPN、连接器详情、导入表和概览刷新上的 `adaptiveGlassButton()` 换成铣削胶囊（`InstrumentButtonStyle`）。安静态是凹槽，主操作填色并在按下时下沉 2pt，悬停时描边扫光走一圈就停。设置里的「打开方式」和代理上游选择从系统菜单换成同一只凹槽（`InstrumentMenuLabel`）。流量页的模式、筛选、对话页签和「简洁 / 完整」收成滑动胶囊；对话搜索和 ⌘K 搜索条用字段凹槽。
- **所有会变的数字都走灵动岛 token 的逐位滚动**：公共方法是 `View.rollingNumber()`（`Views/Shared/Interaction.swift` 的 `RollingNumberModifier`，`.numericText(countsDown:)`，减弱动效时改为不动）。桌面、popup、灵动岛上的计数、百分比、token、金额、瓦特、端口、电量、汇率都加在渲染这个数字的 `Text` 上，句子里只滚数字、周围文案不动。小组件是另一个编译目标，用同一套过渡的 `widgetRollingNumber()`。不给整段正文、路径、版本号加。**这一轮补齐了漏网的**：概览会话瓦片的「刚刚 / 5m」（`row.updated`）、代理日志「共 N 行」、流量页删除确认里的「N 条记录」、导入表的「导入选中 (N)」（按钮标题改成自绘 `Text`，否则够不到那个 `Text`）。**同时删掉了给数字叶子加隐式 `.animation(value:)` 的写法**——灵动岛收起的今日 token（`NotchIslandView` 的 `model.usage.today`）、灵动岛用量卡 hero（`IslandComponents` 的 `value`）都是每秒 / 每次轮询就变一次的输出，隐式动画会让事务常驻在飞，每个显示周期都重排整个 hosting view，而 `.numericText` 自己就是动画（见 [技术 §8](technical/08-performance.md)）。这两处已进回归测试；同一个写法还留在未挂载的 `MetricTile`（`Tile.swift`）里，也已去掉。
...
- **连接卡片可点，弹出「连接」**（`ConnectionDetailPanel` 重写）：一张卡同时是四个不同的说法（Wi-Fi / 隔空投送 / 以太网 / 耳机），所以"点开看到什么"必须对四者是一个意思。**当时的形态**是四块：① 这条流量走的路（无线电 → 本机 → 蓝牙设备，以太网接入时左侧换成以太网）；② 链路质量（RSSI 刻度 + 弱/强两端标注）；③ 本机代理这一跳（`127.0.0.1:<port>/v1`、是否在监听、以及设置页那颗一模一样的检测按钮 —— 这是整台机器上唯一由本应用拥有的一条连接）；④ 蓝牙设备（每台设备的电量与连接方式）；底部两个出口 `流量明细` 与 `打开网络设置`。**这一版已作废**：上面「『连接』详情重做成仪表盘」换成了四只环 + 复制诊断，底部两个出口也去掉了。
...
- **「连接器」页头按钮灰得像禁用**：`headerControl()` 的井与描边本来就是控件的全部表面，但按钮没有 `.buttonStyle(.plain)`，系统默认按钮样式又在胶囊井**内部**画了一个灰色圆角矩形，两层灰叠起来就读成「不可点」。改用 `.plain`，禁用态改为显式 `opacity`，只在真禁用时变淡。
- **页头右上角的圆弧去掉**：概览和连接器标题右上角画着一段半圆轨道加一个圆点，读数已经写在标题旁边，圆弧本身没有含义，还会压住构成条和按钮。概览改回和会话、用量一样的纯标题（数字和刷新还在，不再包进卡片）。连接器和模型的页头带不再画角上的深度环，也不再画这条轨道。模型目录的分区计数改贴在标题旁边——窗口很宽、下面只有一张卡时，`1` / `11` 以前被顶到窗口最右边，看起来像飘着的页码。供应商卡片角上的三层环同样拿掉，状态色只留在水洗和徽章上。
- **页头卡片不再抖**：概览 / 连接器 / 供应商三个页面顶部的卡片带有网格卡片的悬停"上浮 2pt"，而 hover 热区跟着卡片一起位移——指针停在卡片下缘那 2pt 里时，卡片上浮把自己从指针下移开（判定为离开）、落回又接住指针（判定为进入），于是每帧反复翻转，看起来就是卡片在抽。修法有两层：`TileSurface` 的命中形状钉在**未上浮**的几何上（`.contentShape` 排在 `.offset` 之前），指针区域不再随位移走；整宽的**页头带**再显式关掉上浮（`PageHeaderCard` 传 `lift: false`），只保留水洗、内嵌白环与悬停描边点亮——一页只有一条带、控件又在下半部，上浮本来也没传达什么。角上的深度环后来也从这条带上拿掉了，见上面「页头右上角的圆弧去掉」。网格里的卡片手感不变。`Tests/inflight-animation-regressions.py` 把这两条源码性质都钉住了。
...
- **菜单栏状态条加上电池格**：VPN 格右侧多一个电池图形与百分比（当时是 28×15、带正极头；现为 34×21 无正极头的黄金矩形胶囊，见上），充电时液面缓慢起伏（尊重「减弱动效」）；悬停能看到「电量 N% · 充入电池 · 16 W（估算）」。只在 VPN 运行时占用位置。
