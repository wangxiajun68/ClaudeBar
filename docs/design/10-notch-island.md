...
- 右下角滚动卡片是**固定盒子**：标题带 12pt + 间距 8，两行 mark 各 55pt（格子＝图标井 24 + 数值 14 + 说明 11，三个数值都由常量给出，说明行即使为空也占位），行间距 8，卡片内容 138pt，加四周 10pt 内边距后总盒 208 × 158pt。四个状态（额度 / 本机 / 当前模型 / 余额）的卡片尺寸完全一致，reel 转动时不会改变任何几何。
- 分页点用 overlay 钉在卡片底边内侧 9pt，不参与卡片布局 —— 卡片内容怎么变，圆点都不动。
- **已知取舍（方案 B）**：会话区仍按键 4.3 随会话数量弹簧变化，卡片在车道内顶部对齐并 `clipped()`。车道 40 / 44 / 90 / 136pt 时都矮于卡片的 158pt，因此 0–2 个会话时分页点会被裁掉，3 个及以上才可见；换来的是卡片自身尺寸永不随内容或车道变化。
