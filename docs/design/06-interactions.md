...
源码里曾另有一套 master-detail 编辑器（`ProviderEditorView` / `CodexProviderEditorView` + `ProviderEditorModel`），没有挂载点，已删除；它独有的四个 per-model 字段先折进了 `ProviderConnectionEditor`，见 [technical/17](../technical/17-ui-audit-backlog.md) §3。
