...
final class VpnManager: ObservableObject {
    ...
    @Published var traffic = VpnTrafficSnapshot()
    /// Down/up speed samples (bytes/s), newest last — drives the chart and
    /// the menu-bar rate readout. Capped at 60 samples (~1 min at 1 Hz).
    @Published private(set) var speedHistory: [(down: Int64, up: Int64)] = []
    /// Latest sampled speeds for cheap readouts (menu bar).
    ...
    private func extractBundledCoreIfNeeded() {
        guard let bundled = Bundle.main.url(forResource: "mihomo-core", withExtension: nil) else { return }
        let dest = FilePaths.vpnCoreBin
        let fm = FileManager.default
        // Re-extract when missing or older than the bundled copy.
        ...
        // 0755 so the core is executable; not setuid.
    ...
    func startCore() {
        ...
        let binPath = FilePaths.vpnCoreBin.path
        guard FileManager.default.fileExists(atPath: binPath) else {
            fail(.coreMissing)
            VpnSystemProxyController.clearSystemProxy() // don't leave a dead proxy set
            return
        }
        guard FileManager.default.isExecutableFile(atPath: binPath) else {
            fail(.coreNotExecutable(binPath))
            return
        }

        writeConfig()
        state = .starting
        log("启动内核：\(binPath) (controller:\(controllerPort), tun:\(prefs.vpnTunEnabled), sysproxy:\(prefs.vpnSystemProxyEnabled))")

        let proc = Process()
        proc.executableURL = FilePaths.vpnCoreBin
        proc.arguments = ["-d", FilePaths.vpnDir.path, "-f", FilePaths.vpnConfigFile.path]

        // Stream stdout+stderr to core.log so a crash leaves the kernel's
        // own fatal message on disk even if the pipe read races.
    ...
    private func writeConfig() {
        ...
        let text = VpnConfigBuilder.build(
            profileText: subscriptions.activeID.flatMap { subscriptions.profileText($0) },
            prefs: prefs)
        do {
            try text.write(to: FilePaths.vpnConfigFile, atomically: true, encoding: .utf8)
        } catch {
            fail(.configWriteFailed(error.localizedDescription))
        }
    }
    ...
    func refreshProxies() async {
        ...
        // Group membership decides "current": a node is current when a group
        // points at it.
        var currentNodes = Set<String>()
        for g in groupsOut { currentNodes.insert(g.current) }

        var proxiesOut: [VpnProxy] = nodeNames.sorted().map { name in
            let d = details[name]!
            return VpnProxy(
    ...
    private func pollConnections() async {
        ...
        traffic.activeConnections = conns.count
        ...
        traffic.totalDown = totalDown
        traffic.totalUp = totalUp
        ...
        if streamStale, let prev = lastConnTotals {
            ...
            if dt >= 0.5 {
                ...
                speedDown = max(0, derivedDown)
                speedUp = max(0, derivedUp)
                traffic.down = speedDown
                traffic.up = speedUp
                if derivedDown > 0 || derivedUp > 0 {
                    speedHistory.append((down: speedDown, up: speedUp))
                    if speedHistory.count > 60 {
                        speedHistory.removeFirst(speedHistory.count - 60)
                    }
                }
